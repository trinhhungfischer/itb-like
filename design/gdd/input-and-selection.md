...
| **Settings / Options** ✅ (`settings-and-options.md`, Designed 2026-07-28) | — | Key bindings, `require_confirm_click`, **and (A13) `click_tolerance_px` + `keyboard_repeat_delay_ms`/`keyboard_repeat_rate_ms`, which must become player-adjustable rather than designer-only knobs** | **Hard** (Alpha tier, system **#28** — corrected from "#25", which is Pilots). This document owns what a binding *does* and this GDD's tuning knobs are its defaults; Settings owns what key it is on, and persists it. Accessibility **A5** requires every binding here be remappable — none may be hardcoded. Accessibility **A13** (added 2026-07-28 by the accessibility-specialist gate) requires the timing knobs be exposed too: `require_confirm_click` helps a player who *misclicks*, but does nothing for a player with tremor who fails the tolerance window itself. |
