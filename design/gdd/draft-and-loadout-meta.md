...
8. **Draft Offer categories (v1: two).** Every generated offer is one of:
   **`NewHeroOffer`** (recruit a specific not-yet-Rostered `HeroDefinition`,
   Rule 5) or **`AbilityUpgradeOffer`** (grant one `AbilityUpgradeInstance`
   ...
   already-full-slot combination is never shown). A third, always-present,
